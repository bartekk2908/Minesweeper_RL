...
def is_guess(state, index):
    if np_sum(state == -1) == state.size - 1:
        return False
...
def get_reward(action, old_state, new_state, num_bombs):
    if new_state[action] == -2:
        return rewards['lose']
    elif np_sum(new_state == -1) == num_bombs:
        return rewards['win']
    elif old_state[action] == new_state[action]:
        return rewards['no_progress']
    else:
        if is_guess(new_state, action):
