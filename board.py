...
class Board:
        ...
        if piece_click:
            ...
            while left_pieces:
                ...
                for neighbour in piece.get_neighbours():
                    if not neighbour.get_has_bomb() and not neighbour.get_clicked() and not neighbour in left_pieces:
                        left_pieces.append(neighbour)
    ...
    def represent_state(self):
        ...
        for row in range(self.size[0]):
            for col in range(self.size[1]):
                piece = self.get_piece((row, col))
                if not piece.get_clicked():     # -1 -> unclicked piece
                    state[row][col] = -1.0
                else:
                    if piece.get_has_bomb():
                        state[row][col] = -2.0   # -2 -> bomb
                    else:
                        state[row][col] = piece.get_num_around()    # 0-8 -> piece with number
        return state
